<template>
        ...
        <div class="bg-gray-100 min-h-[180vh] max-w-4xl w-full flex flex-col justify-start mb-10 rounded-lg shadow-xl px-4 sm:px-6 lg:px-8">
          <div class="m-5 items-center">
            ...
            <div class="ml-3 mt-10 space-y-3">
              ...
              <select name="" v-model="event_type" id="eventType" class="w-full sm:w-9/12 h-12 rounded-lg font-medium shadow-md">
                <option disabled selected value="" class="text-gray-400">Pick your event here</option>
            ...
            <div class="flex justify-center items-center space-x-4">
              ...
              <button type="submit" @click="submitWishlist" class="mt-12 py-2 px-4 bg-blue-300 hover:bg-blue-400 font-semibold text-gray-900 rounded-lg shadow-lg transition-transform duration-300 transform hover:scale-110">Confirm</button>
            </div>
          </div>
        </div>
      </div>
  ...
  export default {
    name: 'AddWishlist',
    emits: ['loginSuccess'], // Declare emitted events
    data() {
      return {
        event_name: '',
        event_type: '',
    ...
    methods: {
      async submitWishlist() {
        ...
        console.log('Sending data:', wishlistData); // Log the data being sent
